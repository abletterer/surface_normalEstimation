...
Dialog_Surface_NormalEstimation::Dialog_Surface_NormalEstimation(SCHNApps* s) :
    m_schnapps(s),
    m_selectedMap(NULL)
{
    setupUi(this);
...
void Dialog_Surface_NormalEstimation::selectedMapChanged()
{
    if(m_selectedMap)
...
void Dialog_Surface_NormalEstimation::addMapToList(MapHandlerGen* map)
{
    list_maps->addItem(map->getName());
}

void Dialog_Surface_NormalEstimation::removeMapFromList(MapHandlerGen* map)
{
...
void Dialog_Surface_NormalEstimation::addAttributeToList(unsigned int orbit, const QString& nameAttr)
{
    QString vec3TypeName = QString::fromStdString(nameOfType(PFP2::VEC3()));
